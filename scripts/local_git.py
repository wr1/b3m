...
def check_sync_status(repo_path: Path, branch: str) -> Tuple[bool, str]:
    ...
    try:
        ...
        ahead = int(subprocess.run(
            ["git", "-C", str(repo_path), "rev-list", "--count", f"origin/{branch}..HEAD"],
            capture_output=True, text=True, check=True
        ).stdout.strip())
...
def main() -> None:
    ...
    g.add_argument("--git", action="store_true", help="Switch to git+ URLs using local branches")
    g.add_argument("--local", action="store_true", help="Switch to local file:// editable installs")
    ...
    base_deps = [
        "treeparse", "statesman", "pydantic", "pyyaml",
        "tomli>=2.3.0", "tomli-w>=1.2.0",
    ]
    ...
    if args.git:
        ...
        for d in DEPS_CONFIG:
            ...
            dep_lines.append(f"{d['name']} @ git+https://github.com/wr1/{d['repo']}.git@{branch}")
        ...
        if bem_p.exists():
            ...
            project.setdefault("optional-dependencies", {})["bem"] = ["ccblade", bem_line]
            print(f"   b3_bem       → local")
        else:
            print(f"   b3_bem       (not found locally)")
